...
<div class="row">
	<div class="col-md-6">
		<div class="form-group{{ $errors->has('pagina') ? ' has-error' : '' }}">
		    {!! Form::label('pagina', 'Seção:') !!}
		    {!! Form::select('pagina[]', $secoes->pluck('titulo','id'), null, ['id' => 'pagina', 'class' => 'form-control', 'required' => 'required', 'multiple','data-multselect']) !!}
		    <small class="text-danger">{{ $errors->first('pagina') }}</small>
		</div>
	</div>
	<div class="col-md-6">
		<div class="form-group{{ $errors->has('ativo') ? ' has-error' : '' }}">
		    {!! Form::label('ativo', 'Ativo') !!}
		    {!! Form::select('ativo',[true=>'Ativo',false=>'Invativo'], null, ['id' => 'ativo', 'class' => 'form-control', 'required' => 'required']) !!}
